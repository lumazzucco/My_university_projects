...
def create_model(args, env):
    ...
    if args.algo == 'ppo':
        ...
        model = SAC("MlpPolicy", env, learning_rate= args.lr )
    else:
...
def load_model(args, env):
    # T4 
    if args.algo == 'ppo':
        model = PPO.load("ppo_cartpole_0.003")
    elif args.algo == 'sac':
        model = SAC.load("sac_cartpole_0.003")
    else:
        raise ValueError(f"RL Algo not supported: {args.algo}")
    return model
...
if __name__ == "__main__":
    ...
    if args.test is None:
        try:
            ...
            # Saving model (T4) 
            if args.algo=="ppo":    
                model.save("ppo_cartpole_0.003")
            else:
                model.save("sac_cartpole_0.003")
